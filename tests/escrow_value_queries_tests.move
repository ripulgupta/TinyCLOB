...
// === `bid_quote_escrow_at_price` exact dual-aggregate tracking ===
...
fun bid_quote_escrow_at_price_matches_single_order_after_partial_fill() {
    ...
    // Cross-check against the per-order query below, which must agree
...
// === `resting_order_escrow` / `resting_order_escrow_by_ticket` ===
...
// Reachable `Some((0, 0))` state (see `order::Order.original_size`'s doc
// comment and `fill_level_ask` in `tiny_clob.move`): a bid's escrow (Quote)
// can hit exactly `0` strictly before its Base side (`original_size -
// fee_basis_accumulated`) is exhausted, so the order is still resting with
// real remaining Base capacity even though `resting_order_escrow_fields`
// reports `(escrow: 0, remaining_size: 0)` (both fields are the same live
// Quote escrow value for a bid -- see the field's own doc comment).
// Distinct from `None` (not resting at all).
//
// NOTE: this does NOT reuse the shared `shortfall_book`/`shortfall_price`
// fixture (`price=5`, `price_scale=10`). Under the `price_scale = scale_lo`
// derivation, `price_scale` is always a power of ten (see `new`'s doc
// comment), so a `price` of `5` against a `price_scale` of `10` is always
// exactly `1/2` -- the cumulative-ceiling target reaches
