...
// `clob_admin_finalize` takes `cap` by value, but must still reject a cap
// minted for a different book — taking `cap` by value must not weaken this
...
// --- Claim-fees / drain-step ordering around a partially-filled resting
// --- order (see `clob_admin_drain_step`'s and `clob_admin_finalize`'s doc
// --- comments in `sources/tiny_clob.move`). Force-cancelling a
...
// --- already claimed down to zero -- `clob_admin_finalize` sweeps this
// --- residual itself. The three tests below pin down: finalize succeeding
// --- directly on a re-credited residual without any second claim,
// --- reclaiming as an equally-valid alternative, and the trivially-safe
// --- claim-after-drain ordering.
...
/// Regression test for the operational trap where an admin calls
...
/// reclaiming fees a second time. `clob_admin_finalize` must succeed
/// unconditionally and return the correct residual fee amount (2_500 Quote,
/// 0 Base) as a swept coin.
#[test]
...
fun retire_then_drain_step_succeeds_and_retiring_stays_set() {
    ...
    // actual failed unpause attempt before draining -- what this test pins
    // down: after `clob_admin_retire`, `clob_admin_drain_step` succeeds and
    // `retiring` remains `true` throughout.
    cap.clob_admin_drain_step(&mut book, 10, scenario.ctx());
...
// A plain, reversible `clob_admin_pause_book` call alone — never having
// called `clob_admin_retire` — must NOT unlock the destructive
// `clob_admin_drain_step` force-drain path.
#[test]
#[expected_failure(abort_code = 6, location = tiny_clob)] // tiny_clob::ENotRetiring
...
// --- domain event fires, matching each function's doc comment and body in
