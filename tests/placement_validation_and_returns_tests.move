...
fun place_limit_order_zero_price_aborts() {
    ...
    // passing `price == 0` directly triggers, but via genuine
...
fun place_limit_order_size_validation_aborts() {
    ...
    // within the book's declared `[1, 10^19]` range, so `EZeroPrice` cannot
    // also fire here -- an `abort_code`-asserted failure unambiguously proves
...
// 4 entry points gets its own test below: `assert_book_version`/`is_paused`
// runs first in every one of
...
// === `place_market_order_bid`/`ask` surface the max_fills truncation
// signal ===
...
/// `fill_level_bid`'s inner loop checks `level_is_empty()` BEFORE checking
/// `fills_consumed == max_fills` (see `sources/tiny_clob.move`,
/// `fill_level_bid`): if the `max_fills` check ran first, a fill that
/// exactly drains the level's last resting order would increment
/// `fills_consumed` to equal `max_fills`, and the loop would report
/// `stopped_on_max_fills_while_crossing = true` on the next iteration even
/// though the level was completely, naturally drained -- there was no
/// counterparty depth left to truncate against.
///
/// Checking `level_is_empty()` first means a fill that exactly drains the
/// level's last order correctly reports `stopped = false`: the sweep only
/// ran out of counterparty depth, it was never capped short of it. This has
/// a real, user-visible consequence on the bid side:
...
/// resting orders could supply correctly gets `should_rest = true` -- its
/// own leftover legitimately rests, mirroring the "one more than needed"
/// companion test right below. This test pins down that behavior at the
/// boundary so a future accidental change to the check ordering becomes
/// visible.
#[test]
fun place_limit_order_bid_max_fills_exact_match_does_not_falsely_report_stopped() {
    ...
    // Behavior at the exact-match boundary: `stopped` comes back
...
/// `fills_consumed` to `max_fills`. Since `fill_level_ask` checks
