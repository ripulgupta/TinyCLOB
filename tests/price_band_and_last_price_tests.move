...
fun set_last_price_at_or_above_best_bid_succeeds_with_only_bid_present() {
    ...
    // being consistent with a silent no-op (a target value equal to the
    // already-current `last_price` wouldn't distinguish "the call worked"
    // from "the call did nothing").
...
// === `LastPriceSet`, and `set_last_price`'s permissionless / ===
// === pause-and-retire-agnostic surface ===

// --- `LastPriceSet` event fields (`setter`, `last_price`) are otherwise ---
// --- never asserted anywhere in this file. ---
...
// --- `set_last_price` is deliberately not pause/retire-gated -- confirm ---
// --- it actually works on a paused book, and on a retiring one through ---
// --- to `clob_admin_finalize`. ---
...
// --- A non-admin sender can successfully call `set_last_price` without ---
// --- ever constructing or referencing a `ClobAdminCap`. ---
...
// then snaps to a whole tick -- and the band check runs against that
// *snapped* `price`, not the caller's original ratio. The tests below
// hand-derive
