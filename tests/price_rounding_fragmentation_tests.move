...
/// Regression tests demonstrating two rounding/fragmentation properties of
/// `fill_level_bid` (bid taker vs. resting asks).
///
/// `fill_level_bid`/`fill_level_ask` compute each fill's `quote_cost`
/// independently per fill. Ceiling division (`scaled_ceil_mul_div`) is
/// superadditive across separate resting orders at the same price, so a
/// taker sweeping fragmented liquidity could pay strictly more Quote than
/// sweeping one consolidated order for the identical Base delivered, and a
/// bid escrowed with the textbook-exact `bid_escrow_amount` could underfill
/// against a sufficiently fragmented ask book.
///
/// Each fill is classified as MAKER-limited (it fully drains the resting
/// order being matched) or TAKER-limited (the maker still has size left
/// afterward), and only the taker-limited branch rounds up, since that is
/// the only case where the fill's boundary is genuinely set by the taker's
/// own remaining size/budget rather than by fully draining a maker:
///
...
/// maker-limited floors (`max(floor(price * fill_qty / price_scale), 1)`);
/// taker-limited is ceiling. (The mirror-image `fill_level_ask`, for an ask
/// taker vs. resting bids, charges a telescoping cumulative-proportional
/// ceiling -- see `fill_level_ask` in `tiny_clob.move` -- but is not
/// exercised by this file.)
///
/// Every fill in both tests below is maker-limited (each fragmented resting
/// order is always fully drained by a single fill), so both use the floor
/// formula, and fragmentation costs the taker no extra Quote: floor
/// division is NOT superadditive the way ceiling division is (in fact
/// `floor(sum) >= sum(floor)` in general, and here the two sides come out
/// exactly equal). `docs/FEATURES.md`'s `min_size * price / price_scale`
/// sizing guidance remains good practice for keeping any residual dust
/// negligible.
#[test_only]
...
// dividing evenly.
//
// FRAGMENT SIZE: `place_limit_order_ask` derives its resting price from
...
// back exactly `PRICE`. `FRAGMENT_SIZE` (101, not a multiple of
// `price_scale`, so each fragment's own fill still genuinely floor-rounds)
// is the fragment unit used below, and `TOTAL_SIZE` is `FRAGMENT_SIZE *
// FRAGMENT_COUNT`: `FRAGMENT_COUNT` separate maker-limited fills vs. one
// consolidated fill.
...
/// Verifies buying the same `TOTAL_SIZE` (505) Base total costs a taker the
/// SAME Quote whether the resting liquidity is one
...
/// Verifies a resting limit bid escrowed with the textbook-correct
/// `bid_escrow_amount` for a given quantity at a given price fully fills
/// against a fragmented ask-side book -- it does not underfill, because the
/// fragmented book's cumulative maker-limited cost (all floor-rounded)
/// never exceeds the single ceiling-rounded escrow figure.
///
...
fun limit_bid_no_longer_underfills_against_fragmented_book_with_exact_escrow() {
    ...
    assert!(base_received == TOTAL_SIZE, 2); // fills completely
