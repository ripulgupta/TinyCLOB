...
// === Per-item events in `clob_admin_drain_step`'s helpers ===
...
// Covers a mixed bid/bid/bid/ask drain, asserting each `OrderCancelled`
// event's id and trader fields (not just the event count) --
// `clob_admin_drain_step` always fully drains the bid side (in FIFO order
// per level) before the ask side (see `clob_admin_drain_step`'s body), so
// the ask's `OrderCancelled` is deterministically the 4th event.
#[test]
fun clob_admin_drain_step_order_cancelled_events_carry_correct_ids_and_traders() {
    ...
    // stamps BOTH id fields correctly.
...
// test targets (not just "a category boundary lands on a call boundary",
// which every existing test already does trivially by using one oversized
// call):
...
// with `ENotFullyDrained` otherwise; the fee accumulator does not need to be
// pre-drained -- `clob_admin_finalize` sweeps it and returns the swept
