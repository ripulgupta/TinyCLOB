...
// === Fee rounding — ceiling division closes the dust-fee exploit ===
...
fun match_bid_produces_expected_fill_and_fee_amounts() {
    ...
    // matching loop. The maker fee, by contrast, is only SET ASIDE in
...
fun match_ask_produces_expected_fill_and_fee_amounts() {
    ...
    //
    // `expected_quote_cost` (3_495) equals `bid_escrow_amount`'s own
    // ceiling formula for this single, first fill.
...
fun fee_amount_ceiling_rounds_up_dust_and_stays_exact_on_exact_division() {
    ...
    // Fill 999 units: ceil(999 * 10 / 10_000) = ceil(0.999) = 1 — ceiling
    // division collects 1 unit of fee here.
...
/// units yields `50 * 10 / 10_000 = 0.05`, which ceiling division rounds up
/// to 1 rather than down to 0 -- so a taker cannot split a large order into
/// many sub-100-unit fills to pay zero total fee across all of them;
/// repeating such a fill several times collects a clearly nonzero total.
#[test]
fun repeated_dust_sized_fills_now_collect_nonzero_total_fee() {
    ...
    // genuinely sub-100 dust fills, this test uses a bespoke book with a
    // smaller `min_size` instead of shrinking `dust_fill_size` — same
    ...
    // Ceiling division collects 1 unit of fee per fill, so the accumulator
    // reads `num_fills` after this loop.
...
// === `fill_level_ask` sweeping >=2 resting bids in one call, with genuine
// rounding on BOTH bids ===
//
...
// === A 2-maker sweep where ONE maker already carries pre-existing fill
// history from an earlier, separate transaction ===
//
...
// === A `stopped_on_max_fills_while_crossing` sweep resumed by a second
// call ===
//
...
// === `max_fills` exhaustion exactly at a price-LEVEL boundary ===
//
