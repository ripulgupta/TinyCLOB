...
/// Verifies that `order::new`'s "exactly one of the two [escrow_base,
/// escrow_quote]" invariant is actually enforced, not just documented.
/// Since `new` is `public(package)`, any sibling module in this package --
/// including a test module, as here -- can call it directly with BOTH
/// escrow options populated.
///
/// Without that enforcement, this malformed construction would silently
/// succeed, and the resulting order would only fail much later -- and
/// unrecoverably -- the moment it fully drained: `destroy_drained_bid_escrow`
...
///
/// `order::new` asserts `escrow_base.is_some() != escrow_quote.is_some()`
