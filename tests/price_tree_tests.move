...
/// populated, so it cannot be `none` on both sides.
...
/// order — any subtle bug in the tree's insertion path would be very likely
/// to perturb the resulting tree shape/queries for at least one of these
/// permutations.
#[test]
...
/// find-or-insert operation used by `insert_resting_order`: builds a
...
// === insert_or_append_order_regression ===
//
// `insert_or_append_order` performs the whole find-or-insert operation
// within one continuous borrow, with no hint parameter for a caller to pass
// a stale/foreign/mismatched-key pointer for. This is a thorough
// correctness regression test: interleave finds-or-creates at several
// distinct prices, some of them landing at different tree depths, with
// repeat inserts at already-present prices, and confirm every price level
// ends up with exactly (and only) the orders it should contain, with no
// duplication and no lost reachability.
...
// === NO_PARENT blind-spot guard ===
//
// `NO_PARENT` numerically equals `PARTITION_INDEX`, so `is_leaf_ptr(NO_PARENT)`
// (`ptr >= PARTITION_INDEX`) incorrectly evaluates to `true`. Without a
// dedicated guard, passing `NO_PARENT` to `remove`/`borrow`/`borrow_mut`/
// `key` would skip the leaf-shape check and fall through to
// `leaf_index_of(NO_PARENT)` — an out-of-range leaf index that would only
// abort via `sui::table`'s generic missing-key error, not a purpose-built
// one. These four tests confirm each function aborts immediately, with its
