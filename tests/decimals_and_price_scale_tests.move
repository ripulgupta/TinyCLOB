...
// === price_scale/precision/exponent/price_band_factor/last_price ===
...
// === Regression: `affordable_qty` u128->u64 narrowing DoS ===
//
...
// before clamping. This proves the order still succeeds (no abort), with the
// taker's fill correctly bounded by the resting ask's size rather than by
// the overflowed intermediate.
//
// `ASK_SIZE`: any legitimately-derivable nonzero resting-ask price satisfies
// `price * size >= price_scale`, so reproducing `price == 19` exactly at
// this book's `price_scale` (1_000_000_000) requires a size of at least
// `52_631_579`
...
// === Regression: `price_scale` silently landing below `scale_lo` ===
//
// base=30, quote=0, precision=0, exponent=19:
//   scale_lo = 10^30, scale_hi = floor(u64::MAX * 10^30 / 10^19) ~= 1.8e30
//   scale_lo <= scale_hi, so without the `scale_lo <= u64::MAX` conjunct,
//   construction could succeed with price_scale = min(scale_hi, u64::MAX) =
//   u64::MAX, far below the true scale_lo -- silently delivering coarser
//   precision than declared. That conjunct makes construction reject this
//   with EPriceRangeInfeasible instead.
#[test]
#[expected_failure(abort_code = 20, location = tiny_clob)] // EPriceRangeInfeasible
...
// === `price_scale = scale_lo` snaps derived prices to the full declared tick ===
//
// A BTC(8 decimals)/USDC(6 decimals) book with `precision=0` declares only
// whole-dollar true-price resolution, so its raw `price_scale` is the
// SMALLEST value achieving that: `scale_lo = ceil(10^base_decimals *
// 10^precision / 10^quote_decimals) = ceil(10^8 * 10^0 / 10^6) = 100`.
...
// whole-dollar boundary. At `price_scale = 100`, that derivation must snap
// down (bid) or up (ask) to the nearest whole dollar, matching what the
// book's `precision=0` actually declares -- rather than expressing a
// sub-dollar fraction of true price (e.g. $7_900.54...) that would carry
// more precision than the book promises -- while remaining just as
// fund-safe (bid still floors, so escrow never exceeds `payment`; ask still
// ceils, so the filled proceeds never fall short of `expected_quote_output`).
#[test]
fun expected_output_price_snaps_to_declared_tick_under_new_price_scale() {
    ...
    // --- $7_900 -- not a sub-dollar-precise value like $7_900.54... which
    // --- would carry more precision than this book's `precision=0` declares.
    ...
    // --- tick $7_901 -- not a sub-dollar-precise value like $7_901.09...
    // --- which would carry more precision than this book declares.
...
// === Additional declared-range and boundary coverage ===
//
// The tests below cover: `set_last_price`'s declared-range reject side,
// `place_limit_order_ask`'s declared-range reject side,
// `bid_escrow_amount(book, 0, size) == 0`, the `min_size` boundary on a
// `price_scale > 1` book, and the exact `EPriceRangeInfeasible` boundary
// (`scale_lo == scale_hi`).

// --- `set_last_price` declared-range REJECT side ---
//
...
// --- `place_limit_order_ask` declared-range REJECT side ---
//
// Same USDC(6)/BTC(8), precision=1, exponent=0 book shape as the
// `set_last_price` reject-side tests above
...
// --- `bid_escrow_amount(book, 0, size) == 0` ---
#[test]
...
// --- `min_size` boundary on a `price_scale > 1` book ---
//
...
// --- Exact `EPriceRangeInfeasible` boundary (`scale_lo == scale_hi`) ---
//
