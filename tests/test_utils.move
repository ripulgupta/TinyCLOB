...
/// `place_limit_order_bid` does not take an explicit `price` — it derives
...
/// passed alongside `expected_base_output = size`, makes that function
/// derive back exactly `price`.
///
/// The natural candidate is `bid_escrow_amount(price, size) ==
/// ceil(price * size / price_scale)` -- but `floor(escrow * price_scale /
...
/// is the *smallest* payment satisfying the lower bound the derivation
/// needs, if it doesn't round-trip back to `price`, no larger payment can
/// either (a larger payment only derives a `price` >= this one). Aborts if
/// no such payment exists for this exact `(price, size)` pair -- callers
/// hitting this need a different `(price, size)` combination, since this
/// exact one is not constructible without picking a raw `price` directly,
...
public(package) fun assert_extremes_and_adjacent_ticks<Base, Quote>(
    ...
    // `size` must be at least `price_scale` for `place_limit_order_bid`'s
    ...
    // of `price`. A smaller, fixed size cannot reconstruct a small `price`
    // (e.g. `1`) against a large `price_scale` (e.g. `1_000_000`) -- a size
    // of `100` could not derive back `price == 1` when `price_scale ==
    // 1_000_000`. The extremes/adjacent-tick behavior under test is
    // unaffected by the specific size chosen.
    ...
    // round-trip-safe `size` as (b)/(c) above (see the comment on `size`
    // above for why a fixed size would not work for every `price_scale`
    // this helper is exercised against).
    let mid_size = size;
