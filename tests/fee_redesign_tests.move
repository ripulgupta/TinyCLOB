...
/// Tests for fee accounting: the taker fee is deferred and aggregated
...
/// fills. A fill's `quote_cost` formula depends on whether it fully drains
/// the maker:
///   - Fill 1 (ask still has 1 unit left afterward -- taker-limited):
///     `quote_cost = ceil(1_037 * 1 / 100) = ceil(10.37) = 11`.
///   - Fill 2 (fully drains the ask -- maker-limited, uses floor):
///     `quote_cost = max(floor(1_037 * 1 / 100), 1) = max(10, 1) = 10`.
...
/// Taker-side aggregation: 10 separate 1-unit fills against 10
...
/// an exploitably larger amount than what is actually charged. The
/// once-per-call aggregate instead charges `ceil(10 * 10 / 10_000) = 1`, strictly less
...
fun taker_aggregate_fee_strictly_less_than_naive_per_fill_sum_across_many_small_fills() {
    ...
    // Naive per-fill sum: each of the 10 one-unit fills, on its own, would
    // ceiling-round to 1 unit of fee if charged individually instead of
    // once per call.
...
/// `OrderExecuted.taker_fee_amount`, exercised through the real
...
/// Taker-side aggregation, ask-side mirror of
...
fun taker_aggregate_fee_strictly_less_than_naive_per_fill_sum_across_many_small_fills_ask_side() {
    ...
    // Naive per-fill sum: each of the 10 one-unit fills, on its own, would
    // ceiling-round its own 11-unit quote_cost to 1 unit of fee if charged
    // individually instead of once per call.
...
fun cancel_order_with_zero_fills_and_nonzero_fee_refunds_full_escrow_under_realistic_decimals_ask_side() {
    ...
    // Size 52 derives back exactly to `price == 1_037` at this `price_scale`
    // (100) (`expected_quote_output == 539`), which is what this test needs
    // -- not claimed to be the only such size.
    let size = 52;
