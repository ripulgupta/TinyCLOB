...
/// insertion-point search is replayed from an in-memory record of the first
/// descent's path instead of touching `Table` again — see the comments
/// below for why that replay is always exactly a prefix of the first
/// descent's path.
...
/// borrow.
///
/// This function never exposes the recorded descent pointer outside of a
/// single unbroken borrow of `tree`, so there is no gap in which a stale,
/// foreign, or mismatched-key pointer could ever be reused across calls —
/// that class of bug is closed by construction, not by a runtime check.
///
...
/// second `Table`-backed traversal — strictly cheaper than a naive
