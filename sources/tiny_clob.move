...
/// consumed by its own fee (see `fee_amount`'s doc comment). At today's
/// values this is mathematically impossible to violate for any `x > 1` (it
/// would require `bps` close to
...
// `tiny_clob::price_tree`, along with every function that can touch
...
/// policy, valid only as long as every version bump is bookkeeping-only and
/// requires no actual per-book data migration. The day `CURRENT_VERSION` is
...
///
/// Known limitation, accepted as documented rather than fixed: walking
/// `last_price` UP via a
...
/// elsewhere.
///
/// Order-cancellation is mandatory whenever a resting order is found, for
/// two reasons. First, a standalone proceeds-sweep on a still-resting,
/// still-accumulating order would be an unbounded, repeatable exposure: each
/// new fill pools more proceeds, and each subsequent sweep would pay out
/// again, for as long as the order kept resting. Folding in cancellation,
/// together with the `EProceedsCoordinateMismatch` check below (which
/// ensures the cancellation half is actually attempted against the order's
/// true coordinates rather than silently no-op'ing at a mismatched `(side,
/// price)`), guarantees a proceeds sweep is only ever possible either
/// together with the order's cancellation, or when the order is already
/// definitively gone — matching the one-time exposure shape escrow refunds
/// already have. Second, a standalone proceeds-sweep could silently
/// pre-empt the true ticket holder's own `claim_proceeds` call on a
/// perfectly healthy, still-
...
/// cancellation. If neither exists, this is a silent no-op. If both exist,
/// both are settled in the same call, and both `OrderCancelled` and
/// `ProceedsClaimed` are emitted.
///
/// Unconditional — gated only on `book`'s version and `cap`, with no
/// `book.retiring` requirement. The real use case for this function is
/// recovering a resting order's escrow
...
/// book-destroying retirement sequence just to reach it. This carries the
/// same already-accepted risk as any other admin-gated payout here: paying
/// a recorded owner that has drifted from the ticket's true current holder
/// (because `update_resting_order` was never called on a custody change).
/// An integrator that needs
...
/// not per call. `MakerFeeSettled` accounts for up to 2 events per drained
/// order, so an admin choosing `max_items` should size it with that
/// per-order event count in mind.
///
...
/// time resolves it, and `clob_admin_finalize` itself sweeps whatever
...
/// computed once per call, in aggregate, after matching completes (see
...
/// mechanisms avoid the over-collection an independently ceiling-rounded
/// per-fill fee would otherwise cause across many small fills. On the
/// maker-bid side
...
/// which can over-collect across many small fills relative to one large
/// fill of the same total size. `amount` is denominated in Base for a
/// bid-side order, Quote for an
...
/// deliberate: force-cancelling a maker's dust remainder instead of resting
/// it would force-cancel a maker's resting order below their own chosen
/// `min_size` without their consent — a real behavior change this project
/// deliberately keeps out of the core matching engine, which stays a
/// mechanism only and leaves size-policy decisions to the caller.
    ...
    let is_empty_now;
    {
        ...
        loop {
            ...
            *fills_consumed = *fills_consumed + 1;
            ...
            //   drains the resting ask): floor, not ceiling. Independently
            //   ceiling-rounding every full-drain fill is superadditive
            //   across fragmented resting liquidity, so a taker sweeping N
            //   separate 1-unit asks would pay strictly more Quote than
            //   sweeping one consolidated N-unit ask for the identical Base
            //   delivered, and a bid escrowed with the textbook-exact
            //   `bid_escrow_amount` could underfill against a sufficiently
            //   fragmented ask book. `max(floor, 1)` still guarantees a
            //   genuinely nonzero fill charges at least 1 quote atom,
            //   closing the dust exploit a bare floor would otherwise open.
            ...
            // Taker fee (Base) is not deducted per-fill: the full,
            ...
            // ceiling would otherwise cause.
            matched_base.join(base_out);

            let mut quote_payment = budget.split(quote_cost);
            // Maker fee (Quote, this is an ask-side maker) is likewise not
            // credited to the book's fee accumulator per-fill:
...
fun fill_level_ask<Base, Quote>(
    ...
    let ids = book.book_ids();
    // Note: unlike `fill_level_bid`, this function does not read
    ...
    let is_empty_now;
    {
        ...
        loop {
            ...
            *fills_consumed = *fills_consumed + 1;
            ...
            // "fairness fix" for the maker-limited case): applying a
            // ceil/upper-bound conversion uniformly to every fill is broken
            ...
            // taker-limited fills close to their own fair (isolated-ceil)
            // value, which is what minimizes (though, being an apportionment
            // problem, does not perfectly eliminate) how much rounding slack
            // the final fill absorbs relative to a hypothetical isolated
            // trade.
            ...
            // Taker fee (Quote) is not deducted per-fill -- see the
...
public struct OrderExecuted has copy, drop {
    ...
    /// (Entry point values 4 and 5 are never produced by this module and
    /// are not reused for anything else.)
    entry_point: u8,
    ...
    /// (`taker_side == false`). Computed once, across the whole call's
    /// aggregate matched amount, not per-fill, so many small fills within one
    /// call cannot over-collect fees relative to one large fill of the same
    /// total size.
...
/// Quote budget -- there is no separate `budget` parameter; the coin itself
/// is the budget, `max_quote_in` merely bounds how much of it this call may
/// spend.
///
/// - `max_base_out` (`u64`, `0` = a real zero cap that immediately no-ops
///   with nothing matched; `u64::MAX` = unbounded) caps how much Base this
///   call will ever deliver. Like `min_base_out`, this is a NET (post-fee)
///   cap: it bounds the Base the taker actually receives, after the taker
///   fee is deducted, not the gross amount matched against the book.
///   Internally, the GROSS bound fed to matching is inflated from
///   `max_base_out` (via `gross_size_bound_for_net_cap`) so that the
///   post-fee net delivered amount never exceeds it -- so `min_base_out ==
///   max_base_out` is always satisfiable when fully filled, regardless of
///   `taker_fee_bps`.
/// - `max_quote_in` (`u64`, same `0`-is-real / `u64::MAX`-is-unbounded
///   convention) is the taker's spend cap, enforced up front, by
///   construction -- at most `min(payment.value(), max_quote_in)` is ever
///   escrowed/spent, so there is nothing to assert post-hoc and no way for
///   matching itself to spend past it.
...
/// returned untouched; `u64::MAX` = unbounded), is how much Base this call
/// sells; any of `payment` beyond that cap is returned unspent.
/// `min_quote_out` (`0` = not applicable) is the slippage floor. There is no
/// `budget`-equivalent parameter here, since an ask is never budget-gated;
/// `max_base_in` is enforced up front, by construction (via the
/// `min(payment.value(), max_base_in)` clamp below), so there is nothing
/// left to check after the fact.
///
